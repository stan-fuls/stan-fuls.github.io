...
通过 GitHub API 从独立仓库 `stan-fuls/knowledge-docs` 读取 Markdown 文档，在 Web 端提供两种查看模式：

| 视图     | 说明                                                   |
| -------- | ------------------------------------------------------ |
| 时间段   | 按创建时间分组（本月 / 上月 / 近三月 / 近半年 / 更早） |
| 标签归档 | 按 `tags` 字段自动分组，已有标签归入，新标签自动建组   |
...
2. 支持 **两种查看方式**，点击工具栏 `📅 时间段` / `🏷️ 标签归档` 切换：
   - **时间段视图**（默认）：按本月 / 上个月 / 近三月 / 近半年 / 更早分组
   - **标签归档视图**：根据每个文档的 `tags` 字段自动分组
     - 已有标签 → 文档直接归入该标签分组
     - 新标签 → 自动创建新的标签分组
     - 无标签 → 归入「未分类」
     - 一个文档有多个标签 → 同时出现在多个分组中
